...
### 🛠️ ESP32 펌웨어 설치 (필수)
Blossom 로봇 제어를 위해서는 ESP32 펌웨어에 커스텀 헤더 파일이 필요합니다.

1. **파일 준비**:
   - `esp32-voice.yaml`: ESPHome 설정 파일
   - `udp_control.h`: UDP 제어 헤더 파일 (이 저장소의 root에 포함됨)

2. **파일 복사**:
   - Home Assistant의 **File Editor** 또는 **Samba**를 사용하여 `config/esphome/` 폴더로 이동합니다.
   - `udp_control.h` 파일을 해당 폴더에 업로드(또는 생성)합니다.
   - `esp32-voice.yaml` 내용도 업데이트합니다.

3. **설치**:
   - ESPHome 대시보드에서 `esp32-voice` 항목의 **Install** 버튼을 눌러 펌웨어를 빌드하고 업로드합니다.
